...
void DenseCRFLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_EQ(pos_w_.size(), pos_xy_std_.size())<< "pos_w and pos_xy_std should have the same size.";
  CHECK_EQ(bi_w_.size(), bi_xy_std_.size())<< "bi_w and bi_xy_std should have the same size.";
  CHECK_EQ(bi_w_.size(), bi_rgb_std_.size())<< "bi_w and bi_rgb_std should have the same size.";

  CHECK_GE(bottom.size(), 2)<< "bottom must have size larger than 2 (i.e., DCNN output and image dim).";
  CHECK_LE(bottom.size(), 3)<< "bottom size is at most three.";
  if (bottom.size() == 2) {
    has_image = true;
    CHECK_GT(bi_w_.size(), 0)<< "has image as input, but no bilateral parameters specified.";
    CHECK_EQ(bottom[1]->channels(), 3)<< "Can Only support color images for now.";
  } else {
    has_image = false;
  }
...
template<typename Dtype>
void DenseCRFLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_EQ(bottom[0]->num(), bottom[1]->num())<< "The DCNN output and data should have the same number.";
//  CHECK_EQ(bottom[1]->num(), bottom[2]->num())<< "The data and data dimension should have the same number.";
...
template<typename Dtype>
void DenseCRFLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  if (has_image) {
    CHECK_EQ(bottom[0]->height(), bottom[1]->height())<< "DCNN output after upsampling should have the same height as image.";
...
template<typename Dtype>
void DenseCRFLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down,
...
void DenseCRFLayer<Dtype>::SetupUnaryEnergy(const Dtype* bottom_data) {
  ...
  for (int j = 1; j < M_; ++j) {
    for (int k = 0; k < spatial_dim; ++k) {
      scale_data[k] = std::max(scale_data[k], bottom_data[j * spatial_dim + k]);
    }
  }

  // subtraction
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, spatial_dim, 1, -1., sum_multiplier_.cpu_data(), scale_data, 1.,
      norm_data);
  ...
  // sum after exp
  caffe_cpu_gemv<Dtype>(CblasTrans, M_, spatial_dim, 1., norm_data, sum_multiplier_.cpu_data(), 0., scale_data);

  // division
  for (int j = 0; j < M_; ++j) {
    caffe_div(spatial_dim, norm_data + j * spatial_dim, scale_data, norm_data + j * spatial_dim);
  }
