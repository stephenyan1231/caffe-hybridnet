...
void MeanfieldIteration<Dtype>::SetUp(Blob<Dtype>* const unary_terms,
    Blob<Dtype>* const softmax_input, Blob<Dtype>* const output_blob,
  ...
  pairwise_.ReshapeLike(*unary_terms);
  sum_bottom_vec_.push_back(&pairwise_);
  LOG(INFO)<<"unary_terms shape "<<unary_terms->shape_string();
  LOG(INFO)<<"pairwise_ shape "<<pairwise_.shape_string();
...
template<typename Dtype>
void MeanfieldIteration<Dtype>::Reshape(Blob<Dtype>* const unary_terms,
    Blob<Dtype>* const softmax_input, Blob<Dtype>* const output_blob,
    const shared_ptr<ModifiedPermutohedral<Dtype> > spatial_lattice,
    const Blob<Dtype>* const spatial_norm) {
  ...
  CHECK_EQ(channels_, unary_terms->channels());
...
void MeanfieldIteration<Dtype>::Forward_cpu() {
  ...
  caffe_set<Dtype>(count_, Dtype(0.), message_passing_.mutable_cpu_data());

  for (int n = 0; n < num_; ++n) {
    caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, channels_, num_pixels_,
        channels_, (Dtype) 1., this->blobs_[0]->cpu_data(),
        spatial_out_blob_.cpu_data() + spatial_out_blob_.offset(n), (Dtype) 0.,
...
void MeanfieldIteration<Dtype>::Backward_cpu() {
  ...
  caffe_set<Dtype>(this->blobs_[2]->count(), Dtype(0.),
  ...
  for (int n = 0; n < num_; ++n) {
    caffe_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, channels_, num_pixels_,
        channels_, (Dtype) 1., this->blobs_[0]->cpu_data(),
        message_passing_.cpu_diff() + message_passing_.offset(n), (Dtype) 0.,
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    (*bilateral_lattices_)[n]->compute(
        prob_.mutable_cpu_diff() + prob_.offset(n),
