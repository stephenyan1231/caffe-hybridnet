  ...
  if (init_cpu)
    ...
    for (int blob_id = 0; blob_id < this->blobs_.size(); ++blob_id) {
      ...
      if (this->param_propagate_down_[blob_id]) {
        ...
        caffe_gpu_set<Dtype>(cur_blob->count(), Dtype(0), cur_blob->mutable_gpu_diff());

        for (int i = 0; i < num_iterations_; ++i) {
          const Dtype* diffs_to_add = meanfield_iterations_[i]->blobs()[blob_id]->gpu_diff();
          caffe_gpu_axpy<Dtype>(cur_blob->count(), Dtype(1.), diffs_to_add, cur_blob->mutable_gpu_diff());
        }
      }
    }
  }
