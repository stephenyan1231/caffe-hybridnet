...
public:
  explicit HashTableCPU(int key_size, int n_elements) :
      key_size_(key_size), filled_(0), capacity_(2 * n_elements), keys_((capacity_ / 2 + 10) * key_size_), table_(
          2 * n_elements, -1) {
  }
  int size() const {
    return filled_;
...
ModifiedPermutohedral<Dtype>::ModifiedPermutohedral() :
    is_init(false), N_(0), M_(0), d_(0) {
}
...
void ModifiedPermutohedral::init_cpu(const float* features, int num_dimensions, int num_points) {
  ...
  N_ = num_points;
  d_ = num_dimensions;
  HashTableCPU hash_table(d_, N_/**(d_+1)*/);
...
void ModifiedPermutohedral<Dtype>::init_cpu(const Dtype* features, int num_dimensions, int num_points) {
  ...
  // Expected standard deviation of our filter (p.6 in [Adams etal 2010])
  Dtype inv_std_dev = sqrt((Dtype)2.0 / 3) * (d_ + 1);
  // Compute the diagonal part of E (p.5 in [Adams etal 2010])
  for (int i = 0; i < d_; i++)
    scale_factor[i] = 1.0 / sqrt(Dtype((i + 2) * (i + 1))) * inv_std_dev;
  ...
  for (int k = 0; k < N_; k++) {
    ...
    // Find the closest 0-colored simplex through rounding
    Dtype down_factor = (Dtype) 1.0 / (Dtype)(d_ + 1);
    Dtype up_factor = (d_ + 1);
    int sum = 0;
    for (int i = 0; i <= d_; i++) {
      int rd;
      Dtype v = down_factor * elevated[i];
      if (ceilf(v) * up_factor - elevated[i] < elevated[i] - floorf(v) * up_factor)
        rd = ceilf(v);
      else
        rd = floorf(v);
...
template<typename Dtype>
void ModifiedPermutohedral<Dtype>::seqCompute(Dtype* out, const Dtype* in, int value_size, bool reverse,
    bool add) const {
  ...
  for (int j = reverse ? d_ : 0; j <= d_ && j >= 0; reverse ? j-- : j++) {
    for (int i = 0; i < M_; i++) {
      ...
      for (int k = 0; k < value_size; k++)
        new_val[k] = old_val[k] + (Dtype) 0.5 * (n1_val[k] + n2_val[k]);
    }
    Dtype *tmp = values;
    values = new_values;
    new_values = tmp;
//    std::swap(values, new_values);
  ...
  Dtype alpha = (Dtype) 1.0 / (1 + powf(2.0f, -d_));
...
#ifdef SSE_PERMUTOHEDRAL
void ModifiedPermutohedral::sseCompute(float* out, const float* in, int value_size, const bool reverse,
...
void ModifiedPermutohedral::sseCompute(double* out, const double* in, int value_size, const bool reverse,
    const bool add) const {
  const int sse_value_size = (value_size - 1) * sizeof(float) / sizeof(__m128) + 1;
  // Shift all values by 1 such that -1 -> 0 (used for blurring)
  __m128 * sse_val = (__m128*) _mm_malloc( sse_value_size*sizeof(__m128), 16 );
...
template<typename Dtype>
void ModifiedPermutohedral<Dtype>::sseCompute( Dtype* out, const Dtype* in, int value_size, bool reverse, bool add) const
{
...
template<typename Dtype>
void ModifiedPermutohedral<Dtype>::compute_cpu(Dtype* out, const Dtype* in, int value_size, bool reverse, bool add) const {
  if (value_size <= 2)
    seqCompute(out, in, value_size, reverse, add);
  else
