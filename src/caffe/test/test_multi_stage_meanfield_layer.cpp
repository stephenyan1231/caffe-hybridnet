...
TYPED_TEST(MultiStageMeanfieldLayerTest, TestSetUp) {
  ...
  dense_crf_meanfield_param->set_theta_alpha(1);
  dense_crf_meanfield_param->set_theta_beta(3);
  dense_crf_meanfield_param->set_theta_gamma(1);
  dense_crf_meanfield_param->set_num_iterations(3);
  shared_ptr<MultiStageMeanfieldLayer<Dtype> > layer(
      new MultiStageMeanfieldLayer<Dtype>(layer_param));
